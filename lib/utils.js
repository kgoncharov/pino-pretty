...
function prettifyContext ({ log, contextKey = CONTEXT_KEY, colorizer = defaultColorizer }) {
  if (contextKey in log === false) return undefined
  if (typeof log[contextKey] !== 'string') return undefined
  return colorizer.context(log[contextKey])
}

function prettifyFn ({ log, contextKey: fnKey = FN_KEY, colorizer = defaultColorizer }) {
  if (fnKey in log === false) return undefined
  if (typeof log[fnKey] !== 'string') return undefined
  return colorizer.context(log[fnKey])
}
