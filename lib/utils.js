...
function prettifyContext ({ log, contextKey = CONTEXT_KEY, colorizer = defaultColorizer }) {
  let level = 0
  let line = ''
  let levelContext = prettifyContextLevel({ log, contextKey, colorizer, level })
  while (levelContext) {
    line = level === 0 ? `${levelContext}` : `${line} :: ${levelContext}`
    level++
    levelContext = prettifyContextLevel({ log, contextKey, colorizer, level })
  }
  return line === '' ? undefined : line
}

function prettifyContextLevel ({ log, contextKey = CONTEXT_KEY, colorizer = defaultColorizer, level = 0 }) {
  let key = `${contextKey}-${level}`
  if (key in log === false) return undefined
  if (typeof log[key] !== 'string') return undefined
  return colorizer.context(log[key])
}
...
function prettifyObject ({
  ...
  let keysToIterate = objectKeys.filter(k => keysToIgnore.includes(k) === false)

  if (excludeContextKeys === true) keysToIterate = keysToIterate.filter(k => !k.startsWith(CONTEXT_KEY))
